...
const AIRDROP_PROJECTS = [
  { name: "Hyperliquid", chain: "Hyperliquid", status: "Active points farm", url: "https://app.hyperliquid.xyz", criteria: "Trade volume on testnet/mainnet" },
...
export async function checkAirdropEligibility(address: string, project: string): Promise<ToolResult> {
  // Specific eligibility checkers per project
  const projectLower = project.toLowerCase();
  try {
    if (projectLower.includes("hyperliquid")) {
      const r = await axios.post("https://api.hyperliquid.xyz/info", {
        type: "userFills", user: address,
      }, { timeout: 10000 });
      const fills = r.data || [];
      const volume = fills.reduce((a: number, f: { sz: string; px: string }) => a + (parseFloat(f.sz) * parseFloat(f.px)), 0);
      return {
        success: true,
        data: {
          project: "Hyperliquid", address,
          totalVolume: `$${volume.toFixed(2)}`,
          fillCount: fills.length,
          eligibility: volume > 100000 ? "🟢 LIKELY ELIGIBLE — significant volume" : volume > 10000 ? "🟡 MAYBE — moderate volume" : "🔴 INSUFFICIENT — need more volume",
        },
      };
    }
    ...
    return {
      ...
      data: {
        project, address,
        note: `Specific eligibility check for ${project} not implemented. Visit project's website to check.`,
        availableCheckers: ["hyperliquid"],
      },
    };
  } catch (e: unknown) {
