...
async function fetchPage(url: string): Promise<ToolResult> {
  try {
    const r = await axios.get(url, {
      headers: { "User-Agent": "Mozilla/5.0 (Open Talons)" }, timeout: 15000, maxContentLength: 500000,
    });
    const html = r.data;
...
export default definePlugin({
  name: "@open-talons/plugin-research",
  version: "1.0.0",
  ...
  agents: [
    defineAgent({
      ...
      icon: "🔍",
      name: "Research Agent",
      systemPrompt: `🔍 Open Talons Research Agent.
- web_search + fetch_page for deep dives
- rss_read for latest crypto/tech news
- Cross-reference multiple sources`,
      allowedTools: ["web_search", "fetch_page", "rss_read", "rss_list"],
    }),
  ],
});
