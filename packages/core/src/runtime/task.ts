// ============================================================
// 🦅 @open-talons/core — Task Runtime (v2)
...
export interface TaskRunnerOptions {
  ...
  /** Reference to Talons instance for sub-agent spawning */
  talons?: { task: (opts: TaskOptions) => Task };
}
...
export class Task extends EventEmitter {
  ...
  async run(): Promise<TaskResult> {
    ...
    let systemPrompt = "You are Open Talons, an AI agent.";
    let tools: LLMTool[] = [];
    let skillIds: string[] = [];
    ...
    if (skillIds.length) {
      ...
      this.emit("skill_loaded", { skills: skillIds });
    }
        ...
        if (!toolDef) {
          ...
          try {
            const ctx: ToolContext = {
              ...
              spawn: this.runner.talons
                ? async ({ agent, prompt, tools: subTools }) => {
                    const sub = this.runner.talons!.task({ agent, prompt, tools: subTools });
